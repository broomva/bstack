...
name: bstack
description: >
  The Broomva Stack — 16 curated agent skills forming a complete AI-native development
  workflow across 6 layers: Foundation (control, governance, harness), Memory (consciousness,
  ...
  content pipeline). Bootstrap any project with Broomva conventions, install all skills with
  one command, check status, validate health. Use when: (1) setting up a new project with
  Broomva conventions, (2) installing the full agent skills stack, (3) checking which skills
  are installed vs missing, (4) validating skill health, (5) user says "bstack", "broomva
  stack", "bootstrap project", "setup broomva workflow", "install all skills", "skills status".
---

# bstack — The Broomva Stack

16 agent skills across 6 layers for complete AI-native development.
...
ROSTER=(agentic-control-kernel control-metalayer-loop harness-engineering-playbook agent-consciousness knowledge-graph-memory prompt-library symphony symphony-forge autoany deep-dive-research-orchestrator skills skills-showcase arcan-glass next-forge alkosto-wait-optimizer content-creation)
INSTALLED=0; MISSING=()
for s in "${ROSTER[@]}"; do
...
echo "bstack: $INSTALLED/${#ROSTER[@]} skills installed"
...
### `bootstrap` — Install all 16 skills
