...
/bstack fleet up <roster>         → launch N coordinating peers into ONE worktree
/bstack fleet status [--fleet id] → per-peer liveness (pid-keyed) + suggestions
/bstack fleet list                → all fleets with counts by liveness class
/bstack fleet down --fleet <id>   → stop + remove a fleet's peers, reclaim state
...
- **AI-native SDLC — twelve plays on the twenty primitives** (≥ 0.41.0): [references/ai-native-sdlc.md](references/ai-native-sdlc.md). Maps Anthropic's *AI-native SDLC playbook* onto existing primitives, with no new primitive. Mechanisms: `bstack intent` (Stage-1 artifact), `bstack plan-drift` (P4 plan sync), `bstack test-lock` (P11: the fix may not edit its failing test), `bstack evals` (P11: configuration regression-tested by behavior, each eval proven to discriminate), `bstack bands` (P11: deterministic control bands → the next intent), `bstack managed-hooks` (P2: which gates survive `allowManagedHooksOnly`). Templates in `references/templates/`.
- **Dep-Chain (P14) applied to people — the ask ledger**: the human half of the dependency chain lives in the workspace at `.control/asks/<arc>.yaml`, checked by `python3 scripts/ask_ledger.py validate <ledger>`. **An ask is created with both halves or it is not well-formed.** The *audit* half (`class`, `gates`, `exhausted`, `preanswerable`) proves the autonomy ladder was climbed before a person was interrupted — it justifies the ask. The *decision* half (`headline`, `stakes`, `why_you`, `options`) is what makes it answerable, on a phone, in ten seconds. `headline` is ≤100 chars, ends in `?`, and carries no `P<n>` primitive reference, `BRO-<n>` ticket id, session slug (`cowardly-shark-43`) or backticked command — a bare PR number is fine, the reader knows what a PR is. `options` is 2–4 entries of `id`/`label`/`consequence`/`reversible` with exactly one `recommended: true`; recommending an irreversible option while a reversible one exists is **rejected**, not warned, because that quietly spends the operator's authority. Write the layer when the ask is written (next to `stamp`, while the session is still alive to receive the answer); `validate <ledger> --require-decision` is the gate, and it belongs both at ask creation and wherever the ledger is served to a human — the closing handback, arc CI. An optional `liveness: {kind: pr-open, ref: owner/repo#N}` lets `ask_ledger.py archive` retire an ask once that PR closes, except for an ask mentioning a secret: an exposed key outlives the PR that revealed it. Spec: `.control/asks/DECISION-LAYER.md`. The corpus that forced it — 37 open asks, 0 blocking, median 13 days old, 0 offering options, 11 whose every referenced PR was already merged — read to the operator on a phone as "I can't understand a thing". Two further fields make the inbox navigable once it has more than a handful of rows: `project` (whose inbox this is; defaults to the arc slug) and `topics` (1–5 tags from the controlled vocabulary in `DECISION-LAYER.md`). They are what collapse a recurring ask into one group — `#linear-tickets` returned 5 asks across 5 separate ledgers that had read as five unrelated rows.
...
> **`bstack wave` (new in this version).** Fills the N>1 × across-session × external-trigger cell of Orchestrate (P19)'s mechanism cube. Use it when you have N independent plan files that can run in parallel without shared mutable file writes (Fanout (P5)). Each plan's frontmatter declares its `worktree` and `branch`; the wrapper validates atomically, creates worktrees, and launches one `claude --bg` per plan. State lives in `~/.cache/bstack/wave/<wave-id>/` (Persist (P12) filesystem-as-state). See the design at `docs/superpowers/specs/2026-05-13-bstack-wave-design.md` (workspace repo) for the full primitive coalescence map. Since 0.39.1 each plan's peer is spawned through `scripts/peer.py`: named `<worktree>-<ticket>-<slug>` (Fanout P5), with `--strict-mcp-config` and `crossSessionInbound: accept` so it runs unattended, and joined to its live session in `wave status` (`LIVE` column, keyed on the real `claude agents --json --all` schema). A plan that needs the project's MCP servers sets `mcp: inherit` under `wave:` (or `BSTACK_PEER_MCP=inherit`); unknown `wave:` keys are rejected by name.

> **`bstack fleet` (≥ 0.40.0).** The other half of the same cell: N peers coordinating in **one** worktree, from a roster. **The routing rule — decide on the worktree axis, and say which cell you picked:** each peer needs its own branch and worktree → `bstack wave dispatch <plan...>`; peers coordinate in one worktree → `bstack fleet up <roster>`; independent in-session subtasks → Fanout (P5) `Agent` calls in one message. A roster entry declares `slug`, optional `ticket`/`prompt`/`role`/`model`/`mcp`, and `owns` (the path globs that peer owns — its lane, quoted into the brief). The ontology the mechanism used to hardcode is declared config, resolved CLI flag > `BSTACK_FLEET_<KEY>` > `~/.bstack/config.yaml` `fleet_<key>` > default: `base`, `peer_contract`, `allowed_tools`, `state_dir`, `ticket_pattern`, `mcp`. State and per-peer briefs live under `~/.cache/bstack/fleet/<fleet-id>/`; `down` never deletes a fleet's directory while any peer is unreclaimed, and `status` renders `unknown` rather than clean when the agent listing cannot be read. Both commands share `scripts/peer.py` — the Fanout (P5) name grammar (`<worktree>-<ticket>-<slug>`), the two unattended-spawn flags (`--strict-mcp-config`, `--settings '{"crossSessionInbound":"accept"}'`), and pid-keyed liveness.
