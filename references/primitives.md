...
8. **Protect the feedback loop from the agent it constrains** — for a bug fix, commit the failing test first with `bstack test-lock commit <test> -m <msg>`, which records a `Test-Lock: <path> sha256=<hex>` trailer; then fix the code, not the test. The PreToolUse hook blocks edits to a locked test and the common history rewrites (`commit --amend`, `rebase`, a `reset` to or before the lock commit, `branch -f`, `update-ref`, …) — best effort. `bstack test-lock verify` is the gate: it fails when a locked test's content differs from its content at the lock commit (an edit later restored to that content is only a `touched_and_restored` warning), and when the lock commit itself was rewritten (its content no longer matches the recorded sha256). It exits 3 on any `Test-Unlock:` trailer in range, however it was spelled, until a human accepts that commit with `--accept-unlock <sha>` — so no unlock passes unattended. The limit: a reset that drops the lock commit entirely leaves nothing to verify, and is visible only in review.
9. **Configuration is regression-tested by behavior** — a change to `CLAUDE.md`, `AGENTS.md`, skills or hooks runs the agent eval suite (`bstack evals run`), and every production incident becomes an eval. An eval that cannot fail is not an eval: `bstack evals validate --prove` requires each eval's reference to pass, each named `violations` arm to fail a check, and a no-op agent that only claims "I have completed the task." to fail. The agent under test runs in a standalone scratch repo with `--tools` and `--permission-mode dontAsk`, so its `allowed_tools` is its whole grant.
10. **Production feedback is detected deterministically** — `bstack bands` applies Western Electric rules over a rolling baseline, with no model. At 1σ it logs. At 2σ a diagnosis runs with an allowlist of read-only tools (fail closed, `--tools`, `dontAsk`). At 3σ the model may only propose through a PR. The series drops the incomplete current day, and an undersized baseline reports `insufficient_baseline`, never `none`. A breach writes the next `intent.md`.
