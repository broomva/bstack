...
if [ ! -d "$_BSTACK_ROOT/.git" ]; then
    echo "[bstack] v$NEW available (you're on v$OLD). Vendored install: run \`bstack upgrade\` (verified release tarball), or follow $_BSTACK_ROOT/bstack-upgrade/SKILL.md (clones main)."
