...
#   BSTACK_BENCH_LIVE=1 railway run --service YOUR_API_SERVICE -- \\
...
if [ "${BSTACK_BENCH_LIVE:-}" != "1" ]; then
    ...
    echo "  Or:     BSTACK_BENCH_LIVE=1 railway run --service YOUR_API_SERVICE -- bash tests/bench-live.test.sh"
    exit 0
fi
if [ -z "${DATABRICKS_HOST:-}" ] || [ -z "${DATABRICKS_TOKEN:-}" ]; then
    echo "tests/bench-live.test.sh: BSTACK_BENCH_LIVE=1 set but DATABRICKS_HOST/_TOKEN missing; SKIPPING."
    echo "  Provide via shell export, .env, or `railway run --service YOUR_API_SERVICE --`."
    exit 0
fi
