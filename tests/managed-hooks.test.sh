#!/usr/bin/env bash
# tests/managed-hooks.test.sh — BRO-2542. Runs the managed_hooks unittest suite under
...
#
# Run from anywhere:
#   bash tests/managed-hooks.test.sh

set -uo pipefail

BSTACK_REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MODULE="tests.test_managed_hooks"
SUITE="$BSTACK_REPO/tests/test_managed_hooks.py"
TOOL_PY="$BSTACK_REPO/scripts/managed_hooks.py"
SHIM="$BSTACK_REPO/bin/bstack-managed-hooks"
MIN_TESTS=27
...
echo "── managed-hooks suite (BRO-2542) ──────────────────────────────────"
...
# The suite imports managed_hooks directly; every consumer invokes it as a process.
# A shebang or syntax fault that breaks the second is invisible to the first.
if python3 "$TOOL_PY" --help >/dev/null 2>&1; then
    assert_pass "managed_hooks.py --help runs as a subprocess"
else
    assert_fail "managed_hooks.py --help runs as a subprocess"
fi
if [ -x "$SHIM" ] && "$SHIM" --help >/dev/null 2>&1; then
    assert_pass "bin/bstack-managed-hooks is executable and dispatches"
else
    assert_fail "bin/bstack-managed-hooks is executable and dispatches"
fi
